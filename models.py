...
class ToySuperpositionAE(nn.Module):
    """The original autoencoder model (5 -> 2 -> 5)."""
...
class Faithful_SAE(nn.Module): 
    def __init__(self, input_dim, latent_dim, hidden_dim, k=1, use_topk=True):
        ...
        self.encoder = nn.Parameter(torch.empty(input_dim, latent_dim))  # 5 -> 5
        nn.init.xavier_normal_(self.encoder) 
        self.decoder = nn.Parameter(torch.empty(latent_dim, hidden_dim))  # 5 -> 2
        nn.init.xavier_normal_(self.decoder) 
        # No decoder_bias needed
    
    def components(self):
        a = self.encoder  # (5, 5)
        b = self.decoder  # (5, 2)
        # Each component i is: a[:, i:i+1] @ b[i:i+1, :] -> (5, 1) @ (1, 2) = (5, 2)
        P = torch.einsum('ic, ch -> cih', a, b)  # (5, 5, 2)
        return P 
    
    def effective_encoder(self):
        return self.components().sum(dim=0)  # Sum over components -> (5, 2)
    
    def encode(self, x, use_topk=True):
        latent = torch.einsum('bi,il->bl', x, self.encoder)  # (batch, 5)
    ...
    def forward(self, x): 
        ...
        h = torch.einsum('bl,lh->bh', sparse, self.decoder)  # (batch, 2)
        return h , sparse 