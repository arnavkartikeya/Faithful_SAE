...
def create_scaled_ablation_model(original_model, sae, indices_to_ablate, alpha, device):
    ablated_model = type(original_model)()
    ablated_model.load_state_dict(original_model.state_dict())
    with torch.no_grad():
        original_weights = original_model.fc1.weight.data.T.clone()
        components = sae.components().to(device)
        components_to_ablate = torch.sum(components[indices_to_ablate], dim=0)
        ablated_weights = original_weights - (alpha * components_to_ablate)
...
if __name__ == "__main__":
    ...
    biased_dataset = create_prob_biased_dataset("../../colorized-MNIST/training", 500, bias_strength=0.95, rng_seed=config.seed)
    ...
    unbiased_val_loader = DataLoader(unbiased_dataset, batch_size=32, shuffle=False)
...
#     )
    ...
    print("\n--- 📊 Final Evaluation ---")
    alphas_to_test = np.arange(0.0, 1.1, 0.01)
    model_dict = {}
    model_dict['Original'] = copy.deepcopy(biased_model)
    

    for alpha in alphas_to_test:
        model_name = f'Ablated α={alpha:.1f}'
        clean_model_copy = copy.deepcopy(biased_model)
        model_dict[model_name] = create_scaled_ablation_model(
            clean_model_copy, sae_model, [0,1], alpha, device
        )
        
    for m in model_dict.values():
        m.eval()

    print(f"✅ {len(model_dict)} independent models created and ready.")
    ...
    with torch.no_grad():
        for images, labels, colors in unbiased_val_loader:
            ...
            preds = {name: torch.max(m(batch), 1)[1] for name, m in model_dict.items()}
            for i in range(len(labels)):
                label, color = labels[i].item(), colors[i]
    ...
    print("\n--- Scaled Ablation Accuracy Results ---")
    accuracy_data = defaultdict(list)
    for name in model_dict.keys():
        for part in partitions:
            data = results[name][part]
            accuracy = (data['correct'] / data['total']) * 100 if data['total'] > 0 else 0
            accuracy_data[name].append(accuracy)
            
    partition_names = [p.replace('_', ' ').title() for p in partitions]
    df = pd.DataFrame(accuracy_data, index=partition_names)
    ...
    df.to_csv(os.path.join(config.output_dir, 'scaled_results.csv'), index=False)
