...
def create_ablated_model(original_model, sae, device):
    """Creates a new model where the FC1 layer is modified by the SAE."""
    ...
    with torch.no_grad():
        ...
        sae_effective_weights = sae.effective_encoder().to(device)
        ablated_weights = original_weights - sae_effective_weights
...
if __name__ == "__main__":
    ...
    val_loader = DataLoader(BiasedColorizedMNIST('../../colorized-MNIST/testing'), batch_size=32, shuffle=False)
    unbiased_val_loader = DataLoader(UnbiasedColorizedMNIST('../../colorized-MNIST/testing'), batch_size=32, shuffle=False)
    ...
    print("\n--- 📊 Final Evaluation ---")
    ablated_model = create_ablated_model(biased_model, sae_model, device)
